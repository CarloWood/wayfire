    ...
    uint32_t tiled_edges, std::optional<wf::point_t> ws)
{
    ...
    wf::geometry_t const last_windowed_geometry = get_last_windowed_geometry(view).value_or(wf::geometry_t{0,
        0, -1, -1});
    ...
    // Handle unmaximizing:
    wf::geometry_t desired_size = geometry_switch_if(prev_tiled_edges, last_windowed_geometry,
        current_geometry);
    // Handle maximizing:
