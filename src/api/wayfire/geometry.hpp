...
{
    left   -= (tiled_edges & WLR_EDGE_LEFT) ? static_cast<int>(delta.left) : 0;
    right  -= (tiled_edges & WLR_EDGE_RIGHT) ? static_cast<int>(delta.right) : 0;
    bottom -= (tiled_edges & WLR_EDGE_BOTTOM) ? static_cast<int>(delta.bottom) : 0;
    top    -= (tiled_edges & WLR_EDGE_TOP) ? static_cast<int>(delta.top) : 0;
    return *this;
}
